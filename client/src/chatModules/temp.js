...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    const fetchConversations = async () => {
      const res = await fetch(
        `http://localhost:8000/api/conversations/${loggedInUser?._id}`,
        {
          method: "GET",
          headers: {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      const res = await fetch(
        `http://localhost:8000/api/allusers/${user?._id}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
      ...
      <div className="w-screen flex">
        <div className="w-[25%] h-screen bg-secondary overflow-scroll">
          <div className="flex items-center my-8 mx-14">
            <div>
              <img src={`http://localhost:8000${user?.profilePic}`} alt={user?.email}
                width={75} height={75}
                  ...
                  return (
                    <div className="flex items-center py-8 border-b border-b-gray-300">
                      ...
                      >
                        <div>
                          <img
                            src={`http://localhost:8000${user?.profilePic}`} alt={user?.email}
          ...
          {messages?.receiver?.username && (
            <div className="w-[75%] bg-secondary h-[80px] my-14 rounded-full flex items-center px-14 py-2">
              <div className="cursor-pointer">
                <img src={`http://localhost:8000${messages?.receiver?.profilePic}`}
        ...
        <div className="w-[25%] h-screen bg-light px-8 py-16 overflow-scroll">
          <div className="text-primary text-lg">Registered Users:</div>
          <div>
            {users.length > 0 ? (
              users.map(({ userId, user }) => {
                return (
                  <div className="flex items-center py-8 border-b border-b-gray-300" key={user?._id}>
                    <div
                      className="cursor-pointer flex items-center"
